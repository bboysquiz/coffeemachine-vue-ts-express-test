...
const { togglePopup } = useCoffeeMachineStore()
    ...
    <div class="popup">
        <div class="popup__container">
            <img src="../assets/delete.svg" alt="delete" class="popup__close" :class="{ 'hovered': isCloseHovered }"
                @mouseenter="handleCloseMouseEnter" @mouseleave="handleCloseMouseLeave" @click="togglePopup">
            <h2 class="popup__text">Вы успешно сохранили конфигурацию</h2>
            <button :class="{ 'hovered': isButtonHovered }" @mouseenter="handleOkMouseEnter"
                @mouseleave="handleOkMouseLeave" class="popup__button" @click="togglePopup">
                OK
            </button>
        </div>
    </div>
